...
============================================================================
Same pipeline and outputs as saccade_analysis.py (blink masking, AOI
classification, verb-onset-relative first-saccade latency/landing, the
raw-sample-vs-saccade method comparison, and all plots), but the saccade
DETECTOR itself is replaced end to end with the Engbert & Kliegl (2003)
algorithm, following the 7 steps below exactly (matches the lecture slide):
...
Everything downstream of "here is a list of saccade dicts with onset/
offset time, start/end position, peak velocity, amplitude" is UNCHANGED
from saccade_analysis.py: same AOI geometry, same blink handling, same
verb-onset first-saccade measure, same method-comparison logic, same
plots.
...
GROUP_CSV_DIR = (HERE / ".." / ".." / ".." / "01_experiment" / "stimuli"
                  / "creatingDataStructure" / "participant_groups_8").resolve()
...
# AOI fix (2026-08-24, Feiyi): compute_default_aoi_norm() was a formula-derived
# guess that missed pos1/pos4 by 200px+ and sat right on the edge for pos2/pos3
# (verified against the real composed stimulus images). Replaced with the
# pixel-verified positions Jonathan measured directly off the stimuli --
# his numbers are the TOP-LEFT corner of each 440x440 box, confirmed to land
# within a few px of corrected_aoi.py's independently pixel-measured centers.
AOI_TOPLEFT_PX = {
    "pos1": (1891, 429),
    "pos2": (1423, 915),
    "pos3": (697, 915),
...
# ──────────────────────────────────────────────────────────────
# BLINK MASKING (unchanged from saccade_analysis.py)
# ──────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────
# ANNOTATION / TRIAL HELPERS (unchanged)
# ──────────────────────────────────────────────────────────────
...
def plot_method_comparison(comparable, out_path):
    ...
    ax.set_title("Do the Two Methods Agree, Per Trial? (E&K detector)")
    ax.legend(fontsize=8)
    fig.tight_layout(); fig.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"  [plot] saved: {out_path}")
...
def plot_probability_comparison(df, out_path):
    ...
    for ax, cond in zip(axes, conditions):
        ...
        ax.bar(x + width / 2, sacc_means, width, label="Saccade-landing (E&K, count-based)", color="#ff7f0e")
        ax.axhline(0.25, color="#888888", linestyle=":", linewidth=1, label="chance (0.25)")
        ax.set_xticks(x); ax.set_xticklabels(["pos1", "pos2", "pos3", "pos4"])
        ax.set_ylabel("Mean probability"); ax.set_title(cond)
        ax.legend(fontsize=7)
    fig.suptitle("Probability Distribution: Raw-Sample vs. E&K Saccade-Landing Method", y=1.02, fontsize=13, fontweight="bold")
    fig.tight_layout(); fig.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"  [plot] saved: {out_path}")
...
def main():
    ...
    print("\n" + "=" * 90)
    print("PROBABILITY DISTRIBUTIONS: raw-sample vs. E&K saccade-landing method (critical window)")
    print("=" * 90)
    ...
    print("\n" + "=" * 90)
    print("METHOD COMPARISON: E&K first-saccade landing vs. raw-sample critical-window prop_target")
    print("=" * 90)
